...
export default function () {
  ...
  // Step 3: Request AI suggested tasks
  const suggestedTasksPayload = JSON.stringify({
    context: 'I need to improve my productivity',
    goals: ['Complete project', 'Learn new skills'],
    preferences: 'Focus on achievable tasks'
  });

  const suggestedTasksParams = {
    headers: {
      'Content-Type': 'application/json',
  ...
  const suggestedTasksResponse = http.post(
    `${BASE_URL}/api/v1/ai/suggested_tasks`,
    suggestedTasksPayload,
    suggestedTasksParams
  );

  check(suggestedTasksResponse, {
    'ai suggested tasks successful': (r) => r.status === 200,
  });
