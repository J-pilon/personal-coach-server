...
export default function () {
  ...
  const smartGoalPayload = JSON.stringify({
    title: `Test Smart Goal ${Date.now()}`,
    description: 'Performance test smart goal',
    specific: 'Specific goal details',
    measurable: 'Measurable criteria',
    achievable: 'Achievable steps',
    relevant: 'Relevant context',
    time_bound: 'Time bound deadline'
  });

  const smartGoalParams = {
    headers: {
      'Content-Type': 'application/json',
  ...
  const smartGoalResponse = http.post(
    `${BASE_URL}/api/v1/smart_goals`,
    smartGoalPayload,
    smartGoalParams
  );

  check(smartGoalResponse, {
    'create smart goal successful': (r) => r.status === 200 || r.status === 201,
  });
