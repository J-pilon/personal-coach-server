...
export default function () {
  ...
  // Step 3: Make AI proxy request
  const proxyPayload = JSON.stringify({
    messages: [
      { role: 'user', content: 'Test message for performance testing' }
    ],
    model: 'gpt-3.5-turbo'
  });
  ...
  const proxyResponse = http.post(
    `${BASE_URL}/api/v1/ai/proxy`,
    proxyPayload,
    proxyParams
  );

  check(proxyResponse, {
    'ai proxy request successful': (r) => r.status === 200,
  });
