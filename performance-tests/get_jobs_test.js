...
export default function () {
  ...
  // Note: Using a placeholder job ID - may need to create a job first or use a valid ID
  const jobId = '1';
  ...
  const jobResponse = http.get(
    `${BASE_URL}/api/v1/jobs/${jobId}`,
    jobParams
  );

  check(jobResponse, {
    'get job successful': (r) => r.status === 200 || r.status === 404,
  });
